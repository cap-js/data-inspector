sap.ui.define(
  ...
  function (
    ...
    return Controller.extend(
      "sap.cap.datainspector.datainspectorui.controller.dataview",
      {
        onInit: function () {
        ...
        _onPatternMatch: function (oEvent) {
          ...
          let sSelectedColumns =
            oEvent.getParameter("arguments").selectedColumns;
          this._aSelectedColumns = JSON.parse(
            decodeURIComponent(sSelectedColumns)
          );
            ...
            .map(function (obj) {
              if (obj.values && obj.values.length > 0) {
                let aValueFilters = obj.values.map(function (val) {
                  let sVal = val
                    .replace(/^[=*!<>]+|[*!]+$/g, "")
                    .replace(/^(\(|\))|(\(|\))$/g, ""); // Clean the value of any operators or parentheses
                  if (val === "<empty>") {
        ...
        onAdvancedFilterPress: async function () {
          if (!this._oAdvancedFilterDialog) {
            ...
            const fragmentId = this.createId(
              "AdvancedFilterDialog" + Date.now()
            );
