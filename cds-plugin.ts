...
cds.on("bootstrap", (app) => {
  try {
    ...
    const isDevProfile =
      profiles.includes("development") || profiles.includes("dev");
    const isCloudEnv = !!(
      process.env.VCAP_APPLICATION || process.env.KUBERNETES_SERVICE_HOST
    );
    ...
    log.warn(
      "Failed to create route for local static mount of UI5 app.",
      e.message
    );
